...
using Sigurd.Terminal.CommandLine.Help;
...
public sealed class ParseErrorAction : SynchronousCliAction
{
    ...
    public override int Invoke(CliParseResult cliParseResult)
    {
        ...
        {
            WriteTypoCorrectionSuggestions(cliParseResult);
        }

        WriteErrorDetails(cliParseResult);

        if (ShowHelp)
        {
            WriteHelp(cliParseResult);
        }

        return 1;
    }

    private static void WriteErrorDetails(CliParseResult cliParseResult)
    {
        foreach (var error in cliParseResult.Errors)
        {
            cliParseResult.Configuration.Error.WriteLine(error.Message);
        }

        cliParseResult.Configuration.Error.WriteLine();
    }

    private static void WriteHelp(CliParseResult cliParseResult)
    {
        ...
        if (availableHelpOptions.FirstOrDefault(o => o is not null) is { Action: not null } helpOption)
        {
            switch (helpOption.Action)
            {
                ...
                case AsynchronousCliAction asyncAction:
                    asyncAction.InvokeAsync(cliParseResult, CancellationToken.None).GetAwaiter().GetResult();
                    break;
            }
        }
    }

    private static void WriteTypoCorrectionSuggestions(CliParseResult cliParseResult)
    {
        ...
        for (var i = 0; i < unmatchedTokens.Count; i++)
        {
            ...
            bool first = true;
            foreach (string suggestion in GetPossibleTokens(cliParseResult.CommandResult.Command, token))
            {
                if (first)
                {
                    cliParseResult.Configuration.Output.WriteLine(LocalizationResources.SuggestionsTokenNotMatched(token));
                    first = false;
                }

                cliParseResult.Configuration.Output.WriteLine(suggestion);
            }
        }

        if (unmatchedTokens.Count != 0)
        {
            cliParseResult.Configuration.Output.WriteLine();
        }
