...
using Sigurd.Terminal.CommandLine.Completions;
...
public class CliCommand : CliSymbol, IEnumerable
{
    ...
    /// </summary>
    /// <remarks>
    /// <para>Use one of the <see cref="SetAction(Action{CliParseResult})" /> overloads to construct a handler.</para>
    ...
    /// </summary>
    public void SetAction(Action<CliParseResult> action)
    {
        if (action is null)
        {
    ...
    public void SetAction(Func<CliParseResult, int> action)
    {
        if (action is null)
        {
            throw new ArgumentNullException(nameof(action));
    ...
    /// </summary>
    public void SetAction(Func<CliParseResult, CancellationToken, Task> action)
    {
        if (action is null)
        {
            throw new ArgumentNullException(nameof(action));
    ...
    /// </summary>
    /// <remarks>The value returned from the <paramref name="action"/> delegate can be used to set the process exit code.</remarks>
    public void SetAction(Func<CliParseResult, CancellationToken, UniTask<int>> action)
    {
        if (action is null)
        {
            throw new ArgumentNullException(nameof(action));
    ...
    public CliParseResult Parse(IReadOnlyList<string> args, CliConfiguration? configuration = null)
    ...
    /// <param name="configuration">The configuration on which the parser's grammar and behaviors are based.</param>
    /// <returns>A parse result describing the outcome of the parse operation.</returns>
    public CliParseResult Parse(string commandLine, CliConfiguration? configuration = null)
    ...
    public override IEnumerable<CompletionItem> GetCompletions(CompletionContext context)
    {
        ...
        if (context.WordToComplete is { } textToMatch)
        {
            ...
            while (parent is not null)
            {
                ...
                if (context.IsEmpty || context.CliParseResult.GetResult(parentCommand) is not null)
                {
